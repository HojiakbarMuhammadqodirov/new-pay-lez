 ...
 *
 * **The card leads with a day, not with a table.** Under the price sits the one
 * derived figure on the whole section — `subRoundsPerDay`, the tank plus what
 * the clock hands back — with the step up on Free beside it on the two paid
 * cards. Everything under that is the same thirteen rows the section always
 * carried; what changed is that they are now *marked* where they beat the free
 * column, so the offer is legible as a shape before it is read as a list. Both
 * of those are arithmetic on the table above them and neither is a claim: this
 * section has no subscriber count, no "most popular" ribbon and nothing else it
...
function PlanCard({ planIndex, term }: { planIndex: number; term: number }) {
  ...
  const rounds = subRoundsPerDay(planIndex);
  const gain = rounds - subRoundsPerDay(0);
  ...
  return (
    ...
    >
      <div className="sub-card-in">
        ...
        {/*
          * What the money actually buys, in the unit somebody spends an
          * evening in.
          *
          * A tank of 6 against a tank of 4 is a difference nobody can feel from
          * the page. Fourteen more rounds a day is the same fact multiplied out
          * by the refill clock underneath it, and it is the argument the two
          * rows in the strip below were quietly making without ever saying.
          */}
        <div className="sub-day">
          {/* Counted up rather than printed, through the same `[data-count]`
              scan the hero stats and every chart on the site use — it is the
              one figure here a reader is meant to *watch* land, and it honours
              reduced motion in the hook rather than in this file. The `0` is
              what shows if the scan never reaches it. */}
          <b data-count={rounds}>0</b>
          <span className="sub-day-unit">{copy.day.rounds}</span>
          {gain > 0 && (
            <em className="sub-day-up">
              {fill(copy.day.vs, { n: String(gain), plan: copy.plans[0].name })}
            </em>
          )}
          <span className="sub-day-note">{copy.day.from}</span>
        </div>
        ...
        {/* A press per paid card, which there is now something behind. This
            ...
            The free card has no press and never had one, so the slot carries
            the sentence that makes the two chips beside it legible instead: it
            is the column they are counted from. */}
        {plan.id === 'free' ? (
          <p className="sub-base">{copy.day.base}</p>
        ) : (

 ...
 * `learn5.png`): the stats bar, the redeem strip, four game cards, and a
 * leaderboard with two orderings. What is *not* carried over is that app's
 * palette — four games in four different gradients — because this site has one
 * accent. The games are told apart by their icon, their name and their rules,
 * which is what a player actually reads.
 *
 * All four rounds run through one engine. They differ only in how a question is
 * built (`kind` in `GAMES`) and what it pays, so there is one timer, one
 * scoring path and one result card rather than four of each.
 */
  ...
  return (
    <div className="round round-result">
      ...
      <p className="result-score">
        {fill(copy.resultScore, { correct: String(correct), total: String(total) })}
      </p>
...
export function GamesApp() {
  ...
  const start = (id: GameId) => {
    ...
    setQuestions(buildRound(chosen, games));
    setResult(null);
    setPlaying(id);
  };
  ...
  const finish = (correct: number, won: boolean) => {
    ...
    setPlayer(next);
    setResult({ won, correct, points: correct * game.perCorrect });
  };
